...
├── .wiki/                  <-- Single entry for .gitignore / rsync exclude
│   ├── .wiki-state.json    <-- Tracking hashes
│   ├── lint-report.md      <-- Internal quality report
│   └── raw/                <-- Harvested legacy docs
├── wiki/                   <-- "Golden" output (Public branch)
│   ├── index.md
│   └── roles/
├── .wiki-config.yml
├── inventory/
