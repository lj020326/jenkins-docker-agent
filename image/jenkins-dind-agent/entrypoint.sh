...
JAVA_OPTS=${JAVA_OPTS:-""}
AUTOCONFIGURE_DOCKER_STORAGE=${AUTOCONFIGURE_DOCKER_STORAGE:-"false"}
...
then
  ...
  then
    ...
    then
      ...
      elif grep -q aufs /proc/filesystems
...
#        STORAGE_DRIVER="vfs"
  ...
  PING_RESULT="$(curl -sS -X GET --unix-socket /var/run/docker.sock http:/images/_ping)"
  ...
  ## ref: https://support.cloudbees.com/hc/en-us/articles/115001626487-Customize-entrypoint-on-CJE-Agent-Docker-images
  if [[ $# -gt 0 ]]
  then
